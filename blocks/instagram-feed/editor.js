(function(wp) {
    ...
    registerBlockType('golden-hive/instagram-feed', {
        edit: function({ attributes, setAttributes }) {
            ...
            var ops = listOps(images, function(next) { setAttributes({ images: next }); });
            ...
            return el(Fragment, {},
                    ...
                    el(PanelBody, { title: 'Immagini (' + images.length + ')', initialOpen: false },
                        images.map(function(image, index) {
                            return el('div', { key: index, style: { marginBottom: '16px', paddingBottom: '16px', borderBottom: '1px solid #ddd' } },
                                el(ItemControls, { ops: ops, index: index, count: images.length, label: 'Immagine ' + (index + 1) }),
                                el(MediaField, {
                                    value: image.url || '',
                                    onSelect: function(media) { ops.updateMany(index, { url: media.url, urlId: media.id || 0 }); },
                                    onRemove: function() { ops.updateMany(index, { url: '', urlId: 0 }); }
                                }),
                                el(TextControl, {
                                    label: 'Testo alternativo',
                                    value: image.alt || '',
                                    onChange: function(val) { ops.update(index, 'alt', val); }
                                })
                            );
                        }),
                        el(Button, {
                            variant: 'primary',
                            onClick: function() { ops.add({ url: '', alt: '' }); },
                            style: { marginTop: '8px' }
                        }, 'Aggiungi immagine')
                    )
                ),
                el('div', blockProps,
                    el(Placeholder, {
                        ...
                        thumbs: images.map(function(image) { return image.url; })
                    })
                )
            );
        },
