(function(wp) {
    ...
    registerBlockType('golden-hive/whatsapp-button', {
        edit: function({ attributes, setAttributes }) {
            const { phoneNumber, message, buttonText, iconOnly } = attributes;
            const blockProps = useBlockProps();

            return el(Fragment, {},
                el(InspectorControls, {},
                    el(PanelBody, { title: 'Impostazioni WhatsApp', initialOpen: true },
                        el(TextControl, {
                            label: 'Numero di telefono',
                            value: phoneNumber,
                            onChange: function(val) { setAttributes({ phoneNumber: val }); }
                        }),
                        el(TextareaControl, {
                            label: 'Messaggio',
                            value: message,
                        ...
                        el(ToggleControl, {
                            label: 'Solo icona',
                            checked: iconOnly,
                            onChange: function(val) { setAttributes({ iconOnly: val }); }
                        })
                    )
                ),
                el('div', blockProps,
                    el(Placeholder, {
                        icon: el('svg', { viewBox: '0 0 24 24', fill: 'currentColor', style: { color: '#25d366' } },
